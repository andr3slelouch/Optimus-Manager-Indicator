...
function buildPrefsWidget() {
  ...
  let alwaysGpuTempLbl = new Gtk.Label({
    label: _(
      "Always show GPU Temperature, even if Optimus Manager is unavailable"
    ),
    halign: Gtk.Align.START,
  });
  let dynamicHybridLbl = new Gtk.Label({
    label: _(
      "Dynamic icon behaviour in hybrid mode, nvidia icon only if gpu is in use"
    ),
    halign: Gtk.Align.START,
  });
  ...
  let restartGShellLbl = new Gtk.Label({
    label:
      ...
      shellVersion.toString() +
      _(" (alt + F2, type r and hit ENTER) to see the changes"),
    halign: Gtk.Align.START,
  });
  ...
  showGPUMemoryUtilizationSw.set_state(
    settings.get_boolean("show-gpu-memory-utilization")
  );
  ...
  saveButton.connect("clicked", (button) => {
    ...
    settings.set_boolean(
      ...
      showGPUUtilizationSw.get_state()
    );
    settings.set_boolean(
      "show-gpu-memory-utilization",
      showGPUMemoryUtilizationSw.get_state()
    );
  });
  if (shellVersion < 40) {
    grid.show_all();
