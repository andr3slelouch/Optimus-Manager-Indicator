...
const OptimusManagerDialog = GObject.registerClass(
  class OptimusManagerDialog extends ModalDialog.ModalDialog {
    ...
    _detectDistro() {
      ...
      let detectOptimus = GLib.spawn_command_line_sync(
          "/bin/bash -c \"whereis optimus-manager | awk -F: '{print $2}'\""
      );
      detectOptimus = String.fromCharCode.apply(null,  detectOptimus);
      let detectPrimeSelect = GLib.spawn_command_line_sync(
          "/bin/bash -c \"whereis prime-select | awk -F: '{print $2}'\""
      );
      detectPrimeSelect = String.fromCharCode.apply(null,  detectPrimeSelect);
      if (detectOptimus !== "") {
...
export default class OptimusManagerIndicatorExtension extends Extension{
  ...
  enable(){
    this._settings = this.getSettings("org.gnome.shell.extensions.optimus-manager-indicator")
    this.optimusManagerIndicator = new OptimusManagerIndicator(this);
    Main.panel.addToStatusArea('OptimusManagerIndicator', this.optimusManagerIndicator);
      ...
      smi_proc.communicate_utf8_async(null, null, (proc, res) => {
        try {
          ...
          if (proc.get_successful()) {
            ...
            // global.log(JSON.stringify(xmlParsed.nvidia_smi_log, null, 2));
            // global.log(JSON.stringify(xmlParsed.nvidia_smi_log.gpu.utilization.gpu_util, null, 2));
            // global.log(JSON.stringify(xmlParsed.nvidia_smi_log.gpu.utilization.memory_util, null, 2));
            // global.log(JSON.stringify(xmlParsed.nvidia_smi_log.gpu.temperature.gpu_temp, null, 2));
            ...
            // Old code which create a lag every 2s
            ...
            //   let gpuTemperature = gpuValuesArr[2] + " C";
...
function init() {
  ...
  return new OptimusManagerIndicatorExtension();
}
